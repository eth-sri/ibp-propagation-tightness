...
def fc_2layer(normalize, in_ch=3, in_dim=32, width=64, num_class=10):
    model = nn.Sequential(
        normalize,
        nn.Flatten(),
        nn.Linear(in_ch * in_dim**2, 4*width),
        nn.ReLU(),
        nn.Linear(4*width, num_class)
    )
    return model
...
def fc_6layer(normalize, in_ch=3, in_dim=32, width=100, num_class=10):
    model = nn.Sequential(
        normalize,
        nn.Flatten(),
...
def cnn_2layer_bn(normalize, in_ch=3, in_dim=32, width=16, num_class=10):
    model = nn.Sequential(
        normalize,
        ...
        nn.ReLU(),
        nn.Flatten(),
        nn.Linear(in_dim**2 * width, num_class)
    )
    return model
...
def get_network(model_name:str, dataset:str, device, init="default"):
    ...
    elif model_name == "cnn_2layer_bn":
        model = cnn_2layer_bn(normalize, in_ch, in_dim, num_class=num_class)
    elif model_name == "fc_6layer_w50":
        model = fc_6layer(normalize, in_ch, in_dim, width=50, num_class=num_class)
    elif model_name == "fc_6layer_w100":
        model = fc_6layer(normalize, in_ch, in_dim, width=100, num_class=num_class)
    elif model_name == "fc_6layer_w200":
        model = fc_6layer(normalize, in_ch, in_dim, width=200, num_class=num_class)
    elif model_name == "fc_2layer":
        model = fc_2layer(normalize, in_ch, in_dim, width=64, num_class=num_class)
