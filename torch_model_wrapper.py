...
class PGDModelWrapper(BasicModelWrapper):
    ...
    def __init__(self, net, loss_fn, input_dim, device, args):
        ...
        self.current_pgd_weight = args.cert_weight
    ...
    def common_step(self, x, y, eps, num_steps:int, use_vanilla_ibp:bool=None, summary_accu_stat:bool=True, enforce_eval:bool=False):
        ...
        if eps > 0:
            xadv = adv_whitebox(self.net, x, y, (x-eps).clamp(min=0), (x+eps).clamp(max=1), self.device, num_steps, ODI_num_steps=0, lossFunc="pgd", retain_graph=retain_graph)
            yadv = self.net(xadv)
        ...
        loss = (1 - self.current_pgd_weight) * nat_loss + self.current_pgd_weight * adv_loss

        if summary_accu_stat:
            return (loss, nat_loss, adv_loss), (nat_accu.item(), adv_accu.item())
        else:
            return (loss, nat_loss, adv_loss), (nat_accu.item(), adv_accu.item()), (is_nat_accu, is_adv_accu)
...
class BoxModelWrapper(BasicModelWrapper):
    ...
    def _get_pivotal_points_one_batch(self, block, lb, ub, num_steps, C):
        ...
        with torch.enable_grad():
            for re in range(self.args.PGD_restarts):
                # TODO: move this to args factory
                lr = max(0.2, 2/num_steps)
                lr_decay_milestones = [int(num_steps*0.7)] if num_steps >=20 else []
                lr_decay_factor = 0.1
    ...
    def format_return(self, loss, nat_loss, nat_accu, is_nat_accu, cert_loss, cert_accu, is_cert_accu, summary_accu_stat):
        if summary_accu_stat:
            ...
            return (loss, nat_loss, cert_loss), (nat_accu.item(), cert_accu.item()), (is_nat_accu, is_cert_accu)  
